...
namespace Maple.Core.Tests.Repositories
{
    ...
    public class RepositoryTests
    {
        ...
        public void Repository_NEW()
        {
            ...
            Assert.True(repository1 != null);
            Assert.True(repository2 != null);
        ...
        public void Repository_CURD()
        {
            ...
            //插入
            Assert.True (repository1.Insert(user) == true);
            //读取1
            User userRead1 = repository1.Single(id);
            Assert.True(userRead1 != null);            //读取值不为空
            Assert.True(objectIsSame(user, userRead1));

            //修改1
            userRead1.Height = 1000;
            Assert.True(repository1.Update(userRead1) == true);

            //读取2
            User userRead2 = repository1.GetAll().Where(f => f.Id == id).FirstOrDefault();
            Assert.True(userRead2 != null);            //读取值不为空
            Assert.True(objectIsSame(userRead1, userRead2));

            //修改2
            userRead2.Address.Number = 1000;
            Assert.True(repository1.Update(userRead2) == true);

            //删除1
            Assert.True(repository1.Delete(userRead2) == true);

            //新增或修改
            Assert.True(repository1.InsertOrUpdate(userRead2) == true);

            //读取3
            User userRead3 = repository1.GetAllList().FirstOrDefault(f => f.Id == id);               //不推荐使用
            Assert.True(userRead3 != null);                                                           //读取值不为空
            Assert.True(objectIsSame(userRead3, userRead2));

            //删除2
            Assert.True(repository1.Delete(f => f.Id == id) == 1 );

            //新增或修改
            Assert.True(repository1.InsertOrUpdate(userRead2) );

            //查询
            User userRead4 = repository1.GetAll().Where(f => f.IsDeleted == userRead2.IsDeleted && f.Id == userRead2.Id).FirstOrDefault();
            Assert.True(userRead4 != null);                                                           //读取值不为空


            //删除3
            Assert.True(repository1.Delete(id)  );
        ...
        public void Repository_Query()
        {
            ...
            Assert.True(mapleQueryable.GetAll().Where(f => f.CreationTime < DateTime.Now).OrderBy(f => f.CreationTime).ThenBy(f => f.Id).Min(f => f.Id)== 0);
            Assert.True(mapleQueryable.GetAll().Where(f => f.CreationTime < DateTime.Now).OrderBy(f => f.CreationTime).ThenBy(f => f.Id).Sum(f => f.Id) == (0 + allCount - 1) * allCount / 2);
            Assert.True(mapleQueryable.GetAll().Where(f => f.CreationTime < DateTime.Now).OrderBy(f => f.CreationTime).ThenBy(f => f.Id).Average(f => f.Id) == ((double)(0 + allCount - 1)) / 2);

            Assert.True(mapleQueryable.GetAll().Where(f => f.CreationTime < DateTime.Now).OrderBy(f => f.Id).Top(10).Select().Count == 10);

            Assert.True(mapleQueryable.GetAll().Where(f => f.CreationTime < DateTime.Now).OrderBy(f => f.Id).Range(10, 20).Select().Count == 10);

            IPagedList<User> paged = new MaplePagedList<User, long>(mapleQueryable.GetAll().OrderBy(f => f.Id), 5, 10);
            Assert.True(paged.Count == 10);
            Assert.True(paged.First().Id == 40);
            Assert.True(paged.Last().Id == 49);
